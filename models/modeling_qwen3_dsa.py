...
def fp16_index(q, weights, k):
    ...
    index_score = torch.einsum('bsnd,btd->bsnt', q, k) #(bsz, seqlen, n_heads, seqlen_k)
...
class Indexer(nn.Module):
    ...
    def update(self, k, start_pos, end_pos):
        ...
        if self.k_cache is None:
            self.k_cache = torch.zeros(bsz, self.max_seq_len, self.head_dim, dtype=k.dtype, device=k.device)
        self.k_cache[:, start_pos:end_pos] = k

    def forward(self, x: torch.Tensor, start_pos:int, end_pos:int, **kwargs) -> torch.Tensor:
        ...
        weights = weights.unsqueeze(-1)  * self.softmax_scale #(bsz, seqlen, n_heads, 1)
        ...
        mask = torch.full((seqlen, seqlen_k), float("-inf"), device=x.device).triu_(1) if seqlen > 1 else None
        if mask is not None:
            index_score += mask

        topk_indices = index_score.topk(min(self.index_topk, index_score.shape[-1]), dim=-1)[1]
...
class Qwen3DSAAttention(Qwen3Attention):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        cos, sin = position_embeddings
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin) #bsz, n_heads, seqlen, head_dim
        ...
        mask = torch.full(mask_shape, float("-inf"), device=hidden_states.device).triu_(1) if mask_shape[-2] > 1 else None
        index_mask = torch.full(mask_shape, float("-inf"), device=hidden_states.device).scatter_(-1, topk_indices, 0)
        if mask is not None:
            index_mask += mask
...
def load_qwen3_dsa_from_pretrained(model_name_or_path, config=None, **kwargs):
    ...
    assert torch.allclose(custom_model.model.embed_tokens.weight, base_model.model.embed_tokens.weight, atol=1e-6)
